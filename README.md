...
6) Методы класса ([в логическом порядке](https://kotlinlang.org/docs/coding-conventions.html#class-layout). Например, метод располагается после того, в котором впервые упомянут). 
7) inner классы
8) companion object
